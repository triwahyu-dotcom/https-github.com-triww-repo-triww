...
import { VendorIntakePayload, resubmitVendorRevision, validateVendorIntakePayload } from "@/lib/vendor/store";
...
export async function POST(
  ...
  const payload: VendorIntakePayload = {
    ...
    bankName2: String(body.bankName2 ?? "").trim(),
    bankAccountNumber2: String(body.bankAccountNumber2 ?? "").trim(),
    bankAccountHolder2: String(body.bankAccountHolder2 ?? "").trim(),
  ...
  try {
    const vendor = await resubmitVendorRevision(token, payload);
